...
def get_attention_map(
    ...
    attn = outputs.attentions[layer % num_layers]
...
def main():
    ...
    # ══════════════════════════════════════════════════════════
    # Part 1：注意力可视化（使用 eager 后端单独加载模型）
    # ══════════════════════════════════════════════════════════
    print("\n── Part 1：注意力权重可视化 ──")

    from transformers import AutoTokenizer, EsmModel
    attn_tokenizer = AutoTokenizer.from_pretrained("facebook/esm2_t6_8M_UR50D")
    esm_for_attn   = EsmModel.from_pretrained(
        "facebook/esm2_t6_8M_UR50D",
        attn_implementation="eager"
    ).to(device)
    ckpt = torch.load(ckpt_path, map_location=device)
    esm_for_attn.load_state_dict(ckpt['esm_state'])
    esm_for_attn.eval()
    print("注意力可视化模型加载完成 ✓")
    ...
    for lbl, seq in selected.items():
        ...
        attn_map, tokens = get_attention_map(
            esm_for_attn, attn_tokenizer, seq, device, layer=-1)
        plot_attention(
            attn_map, tokens,
...
# Loading weights: 100%|█████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████| 102/102 [00:00<00:00, 1793.25it/s]
...
# ── Part 1：注意力权重可视化 ──
# Loading weights: 100%|█████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████| 102/102 [00:00<00:00, 2487.75it/s]
# [transformers] EsmModel LOAD REPORT from: facebook/esm2_t6_8M_UR50D
# Key                       | Status     | Details
# --------------------------+------------+--------
# lm_head.dense.weight      | UNEXPECTED |
...
# - MISSING:      those params were newly initialized because missing from the checkpoint. Consider training on your downstream task.
# 注意力可视化模型加载完成 ✓
#   处理 Mitochondria，序列长度 512
# 注意力图已保存：week11/day7/attention_mitochondria.png
#   处理 Nucleus，序列长度 82
# 注意力图已保存：week11/day7/attention_nucleus.png
#   处理 Extracellular，序列长度 227
# 注意力图已保存：week11/day7/attention_extracellular.png

# ── Part 2：t-SNE 嵌入可视化 ──
# 提取 Fine-tuned 嵌入...
#   运行 t-SNE（500 条样本）...
#   t-SNE 图已保存：week11/day7/tsne_finetuned.png
# 重新加载 Frozen ESM-2 权重...
# Loading weights: 100%|█████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████████| 102/102 [00:00<00:00, 2543.56it/s]
# [transformers] EsmModel LOAD REPORT from: facebook/esm2_t6_8M_UR50D
# Key                       | Status     | Details
# --------------------------+------------+--------
# lm_head.dense.weight      | UNEXPECTED |
...
# 提取 Frozen 嵌入...
#   运行 t-SNE（500 条样本）...
#   t-SNE 图已保存：week11/day7/tsne_frozen.png

# ── Week 11 Day 7 完成 ──
# 生成文件：
#   week11/day7/attention_extracellular.png
#   week11/day7/attention_nucleus.png
#   week11/day7/attention_mitochondria.png
#   week11/day7/tsne_finetuned.png
#   week11/day7/tsne_frozen.png


# Q1：注意力热图中，对角线（位置 i 关注自身）通常比较亮，这说明什么？如果某个区域有一片连续的亮色块，可能代表什么生物学结构？
# 对角线亮色说明每个氨基酸位置在计算注意力时对自身的关注度较高，这可能反映了蛋白质序列中局部结构的重要性。
# 连续的亮色块可能代表蛋白质中的二级结构元素，如 α-螺旋或 β-折叠，这些区域通常具有较强的局部相互作用和功能相关性。

# Q2：Fine-tuned 和 Frozen 的 t-SNE 图，哪些类别分离得最好？哪些类别最容易混在一起？与 Day4/Day6 的混淆矩阵结论是否一致？
# Fine-tuned 的 t-SNE 图中，Extracellular 类别分离得最好，而 Nucleus 和 Cytoplasm 类别最容易混在一起。与 Day4/Day6 的混淆矩阵结论一致，模型在这些类别上的区分能力较弱。

# Q3：t-SNE 只取了 500 条样本，而不是全部 1259 条，原因是什么？如果把 perplexity 从 30 改成 5 或 100，图会有什么变化？
# t-SNE 的计算复杂度较高，处理全部 1259 条样本可能会导致计算时间过长或内存不足，因此只取了 500 条样本以加快可视化速度。
# 如果将 perplexity 从 30 改为 5，t-SNE 会更关注局部结构，可能导致类别之间的分离不明显，图中点的分布可能更加紧密和局部化；
# 如果将 perplexity 改为 100，t-SNE 会更关注全局结构，可能导致类别之间的分离更加明显，但也可能引入更多的噪声和不稳定性，图中点的分布可能更加分散和不规则。