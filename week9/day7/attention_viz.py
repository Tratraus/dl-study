...
class Encoder(nn.Module):
    ...
    def forward(self, src, src_key_padding_mask=None):
        x = self.embedding(src) + self.pos_embedding(torch.arange(src.size(1), device=src.device))
        x = self.dropout(x)
...
# ── 复用之前所有模型代码 ──────────────────────────────────────
# Encoder / make_causal_mask / make_batch 已从 Day 5 迁移
# 训练完后我们来提取注意力权重
...
def plot_attention(attn_weights, src_tokens, tgt_tokens, title="Cross-Attention"):
    ...
    plt.tight_layout()
    plt.savefig("week9/day7/attention.png", dpi=150)
    plt.show()
...
# 不是，可能在序列的开头或结尾位置有偏差，因为模型在这些位置的上下文信息较少。
# Q2：average_attn_weights=True 对多头取平均。如果不取平均，每个头的注意力模式会不同吗？
# 在序列翻转任务里，你预期不同的头会关注什么？
# 会所不同
# 例如，在蛋白质-配体对接中，Cross-Attention 可以显示模型
# 在预测配体结合位点时关注蛋白质序列的哪些氨基酸残基。
# 在 RNA 二级结构预测中，Cross-Attention 可以揭示
# 模型如何捕捉 RNA 链内不同位置之间的相互作用，
# 从而预测正确的二级结构。这些可视化结果不仅有助于解释模型的决策过程，
# 还可以提供生物学洞见，指导实验验证。
# Q3：Cross-Attention 可视化在计算生物学中有什么实际应用？（提示：想想蛋白质-配体对接、RNA 二级结构预测）
# Cross-Attention 可视化可以帮助我们理解模型在处理生物序列时关注哪些位置。
# 例如，在蛋白质-配体对接中，Cross-Attention 可以显示模型
# 在预测配体结合位点时关注蛋白质序列的哪些氨基酸残基。
# 在 RNA 二级结构预测中，Cross-Attention 可以揭示
# 模型如何捕捉 RNA 链内不同位置之间的相互作用，
