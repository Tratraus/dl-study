...
class ESMWithLoRA(nn.Module):
    def __init__(self, rank=8, alpha=16, n_lora_layers=2):
        ...
        for i in range(num_layers - n_lora_layers, num_layers):
            ...
            layer.self_attn.q_proj = LoRALinear(layer.self_attn.q_proj, rank=rank, alpha=alpha)
            layer.self_attn.v_proj = LoRALinear(layer.self_attn.v_proj, rank=rank, alpha=alpha)
...
# Epoch  1 | Train 0.7839 | Val 0.7815 | F1-H 0.000 F1-E 0.000 F1-C 0.877 | 0.9s
# Epoch  2 | Train 0.8521 | Val 0.9199 | F1-H 0.000 F1-E 0.880 F1-C 0.954 | 0.3s
# Epoch  3 | Train 0.9194 | Val 0.9199 | F1-H 0.000 F1-E 0.880 F1-C 0.954 | 0.2s
# Epoch  4 | Train 0.9194 | Val 0.9199 | F1-H 0.000 F1-E 0.880 F1-C 0.954 | 0.2s
# Epoch  5 | Train 0.9194 | Val 0.9199 | F1-H 0.000 F1-E 0.880 F1-C 0.954 | 0.2s
# Epoch  6 | Train 0.9195 | Val 0.9199 | F1-H 0.000 F1-E 0.880 F1-C 0.954 | 0.2s
# Epoch  7 | Train 0.9201 | Val 0.9215 | F1-H 0.068 F1-E 0.880 F1-C 0.955 | 0.2s
# Epoch  8 | Train 0.9218 | Val 0.9240 | F1-H 0.202 F1-E 0.880 F1-C 0.956 | 0.3s
# Epoch  9 | Train 0.9244 | Val 0.9259 | F1-H 0.309 F1-E 0.880 F1-C 0.957 | 0.2s
# Epoch 10 | Train 0.9258 | Val 0.9263 | F1-H 0.315 F1-E 0.880 F1-C 0.958 | 0.2s
...
# E. LoRA r=8                  21,443   0.9263  0.315  0.880  0.958       0.3
...
# Val Acc 差异：0.9296 - 0.9263 = 0.0033
# F1-H 差异：0.381 - 0.315 = 0.066
# LoRA 的参数效率较高，因为它使用了更少的参数（约 1/4）就达到了接近的 Val Acc 和 F1-H，说明 LoRA 能够更有效地利用参数来调整模型的性能。

# Q3：LoRA 替换的是 Q/V 投影，而不是 K 投影。你认为为什么通常不替换 K？（提示：从注意力机制的角度思考 Q、K、V 各自的作用）
# Q 负责生成查询向量，K 负责生成键向量，V 负责生成值向量。
# 在注意力机制中，查询向量（Q）与键向量（K）进行点积以计算注意力权重，然后这些权重被用来加权值向量（V）。
# 如果替换 K 投影，可能会导致注意力权重的计算发生较大变化，从而影响模型的性能。
# 而替换 Q/V 投影则更容易调整模型的输出，同时保持注意力机制的稳定性。因此，通常选择替换 Q/V 而不是 K。