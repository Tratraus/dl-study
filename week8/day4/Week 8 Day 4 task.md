# Week 8 Day 4：Adapter vs 解冻微调对比实验

## 今天的任务目标

**在相同数据、相同训练配置下，系统对比四种策略，输出完整的多维度对比表格。**
...
```
1. 相同数据：相同的 train/val split，相同的 random seed
2. 相同训练配置：相同 epoch 数、相同 batch size、相同 lr
3. 相同随机种子：每次实验前重置 torch/numpy/random 的种子
```
...
新建文件：`week8/day4/compare.py`

整体结构：定义四种策略的构建函数 → 统一训练循环 → 汇总输出表格。
