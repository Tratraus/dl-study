...
def main():
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if improved:
            ...
            torch.save(checkpoint, "week6/day5/best_model.pt")
    ...
    final_loss, final_acc = evaluate(model, val_loader, loss_fn, device)
    print(f"\n最终验证集结果：Loss={final_loss:.4f}，Acc={final_acc:.2%}")
    with open("training_log.csv", "w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["epoch", "train_loss", "val_loss", "val_acc", "lr", "improved"]
        )
        writer.writeheader()
        writer.writerows(history)

    print("最佳模型已保存：best_model.pt")
    print("训练日志已保存：training_log.csv")
