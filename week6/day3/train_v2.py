...
def main():
    ...
    # ── Early Stopping + 最优模型保存 ───────────────────────
    early_stopping = EarlyStopping(patience=10, min_delta=1e-4)
    best_model_state = None      # 用来保存最优模型的参数

    # ── 训练循环 ─────────────────────────────────────────────
    print(f"\n{'Epoch':>6}  {'Train Loss':>10}  {'Val Loss':>10}  {'Val Acc':>8}  {'Best':>5}")
    print("-" * 50)

    for epoch in range(1, 101):   # 最多跑 100 个 epoch，Early Stopping 会提前结束
        ...
        if improved:
            # TODO: 保存当前模型参数到 best_model_state
            # 提示：import copy; best_model_state = copy.deepcopy(model.state_dict())
            import copy
            best_model_state = copy.deepcopy(model.state_dict())

        flag = "✅" if improved else ""
        if epoch % 5 == 0 or early_stopping.should_stop:
            print(f"{epoch:>6}  {train_loss:>10.4f}  {val_loss:>10.4f}  {val_acc:>8.2%}  {flag}")

        if early_stopping.should_stop:
            print(f"\nEarly Stopping 触发！在 Epoch {epoch} 停止训练")
            break
